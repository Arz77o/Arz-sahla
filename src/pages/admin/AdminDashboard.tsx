...
import { 
  ...
  TrendingUp, Activity, Award, Download
} from 'lucide-react';
import { 
    ...
    const fetchDashboardData = async () => {
      ...
      try {
        ...
        const [
          ...
          supabaseAdmin.from('orders').select('total_dzd, created_at').gte('created_at', firstDayOfMonth.toISOString()).in('status', ['paid', 'processing', 'shipped', 'delivered']),
          supabaseAdmin.from('users').select('*', { count: 'exact', head: true }),
          supabaseAdmin.from('orders').select('*', { count: 'exact', head: true }).eq('status', 'paid'),
          supabaseAdmin.from('settings').select('profit_per_usd, usd_to_dzd_rate').single()
        ]);
        ...
        const totalProfit = (itemsData as any[])?.reduce((sum, item: any) => {
          const priceUsd = item.products?.price_usd || 0;
          return sum + (item.quantity * priceUsd * profitFactor);
        }, 0) || 0;

        const purchaseCost = (itemsData as any[])?.reduce((sum, item: any) => {
          const priceUsd = item.products?.price_usd || 0;
        ...
        const dailyData = last14Days.map(dateStr => ({
          name: new Date(dateStr).toLocaleDateString('ar-DZ', { day: 'numeric', month: 'short' }),
          revenue: (recentOrders as any[])
  ...
  return (
    <>
      ...
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="text-3xl font-black text-gray-900 tracking-tight">إحصائيات المتجر</h1>
          <p className="text-gray-500 mt-1 font-medium">متابعة الأداء الحي لـ Sahla DZ</p>
        </div>
        <div className="flex gap-2">
          ...
          <Link to="/admin/orders">
            <Button className="bg-blue-600 hover:bg-blue-700 shadow-lg shadow-blue-100">عرض الطلبات</Button>
          </Link>
        </div>
      </div>
          ...
          <div key={i} className="bg-white p-6 rounded-3xl border border-gray-100 shadow-sm hover:shadow-md transition-shadow">
            <div className="flex items-center justify-between mb-4">
              ...
              <div className={`flex items-center text-xs font-bold px-2 py-1 rounded-lg ${
                ...
                {card.growth >= 0 ? <ArrowUpRight className="w-3 h-3 ml-1" /> : <ArrowDownRight className="w-3 h-3 ml-1" />}
                {Math.abs(card.growth)}%
              </div>
            </div>
            <div className="text-sm font-bold text-gray-500 mb-1">{card.label}</div>
            <div className="text-2xl font-black text-gray-900 flex items-baseline gap-1">
              {loading ? <div className="h-8 w-16 bg-gray-100 animate-pulse rounded" /> : (
                <>
                  {card.prefix === 'دج' ? formatDZD(card.value as number) : card.value}
                </>
              )}
            </div>
          </div>
        ...
        <div className="lg:col-span-2 bg-white p-8 rounded-3xl border border-gray-100 shadow-sm">
          <div className="flex items-center justify-between mb-8">
            <div className="flex items-center gap-2">
              ...
              <h3 className="font-black text-gray-900 text-lg">تحليل المبيعات</h3>
            </div>
            ...
            {topProducts.length === 0 ? (
              ...
              <div key={i} className="flex items-center gap-4">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="text-xs text-gray-500 mt-0.5">{prod.value} قطعة مباعة</div>
                </div>
