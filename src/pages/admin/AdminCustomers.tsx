...
import { Loader2, Search, Mail, Phone, Calendar, ShoppingBag, DollarSign, UserCheck, UserX, Eye } from 'lucide-react';
  ...
  useEffect(() => {
    const fetchCustomers = async () => {
      ...
      const { data: ordersData, error: ordersError } = await supabaseAdmin
        .from('orders')
        .select('user_id, total_dzd, status') as { data: any[] | null, error: any };
      ...
      if (!ordersError && ordersData) {
        ...
        setCustomers(usersData.map((u: any) => ({ ...u, orders_count: 0, total_spent: 0 })));
      }
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-right">
            <thead className="bg-gray-50 border-b border-gray-200">
              <tr>
                <th className="px-6 py-4 text-xs font-semibold text-gray-500 uppercase tracking-wider text-right">العميل</th>
                <th className="px-6 py-4 text-xs font-semibold text-gray-500 uppercase tracking-wider text-right text-nowrap">الطلبات</th>
                <th className="px-6 py-4 text-xs font-semibold text-gray-500 uppercase tracking-wider text-right text-nowrap">إجمالي الإنفاق</th>
                <th className="px-6 py-4 text-xs font-semibold text-gray-500 uppercase tracking-wider text-right text-nowrap">الحالة</th>
                <th className="px-6 py-4 text-xs font-semibold text-gray-500 uppercase tracking-wider text-right text-nowrap">التسجيل</th>
                <th className="px-6 py-4 text-xs font-semibold text-gray-500 uppercase tracking-wider text-right">إجراء</th>
              </tr>
            </thead>
                  ...
                  <tr key={customer.id} className="hover:bg-gray-50 transition-colors">
                    <td className="px-6 py-4">
                      <div className="flex items-center">
                        <div className={`flex-shrink-0 h-11 w-11 rounded-xl flex items-center justify-center text-white font-bold text-lg shadow-sm ${
                          customer.role === 'admin' ? 'bg-purple-600' : 'bg-blue-600'
                        }`}>
                          {customer.full_name?.charAt(0).toUpperCase() || customer.email?.charAt(0).toUpperCase()}
                        </div>
                        <div className="mr-4">
                          <div className="text-sm font-bold text-gray-900">{customer.full_name || 'بدون اسم'}</div>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap flex gap-2">
                      ...
                      <button 
                        ...
                        title={customer.is_active !== false ? 'حظر الحساب' : 'تفعيل الحساب'}
                      >
                        {customer.is_active !== false ? <UserX className="w-5 h-5" /> : <UserCheck className="w-5 h-5" />}
                      </button>
                    </td>
                  </tr>
                ))
