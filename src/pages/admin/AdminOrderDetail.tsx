  ...
  useEffect(() => {
    const fetchOrder = async () => {
      ...
      const { data, error } = await supabaseAdmin
        .from('orders')
        .select('*, users(email), order_items(*, products(name_ar, name_en, images, price_usd, price_dzd))')
        .eq('id', id)
        .single();
    ...
    <>
      ...
      <div className="mb-8 flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          <div className={`px-4 py-2 rounded-full text-sm font-bold ${
            order.payment_method === 'cod' ? 'bg-amber-100 text-amber-800' : 'bg-blue-100 text-blue-800'
          }`}>
        ...
        <div className="lg:col-span-2 space-y-8">
              ...
              <div>
                ...
                <div className="flex flex-col gap-2">
                  <div className="inline-flex items-center gap-2 px-3 py-1 bg-gray-100 rounded-lg text-sm font-bold text-gray-800 self-start">
                    {order.shipping_method === 'home' ? 'توصيل للمنزل' : 'استلام من المكتب (Stop Desk)'}
                  </div>
                  {order.yalidine_desk && (
                    <div className="p-3 bg-amber-50 border border-amber-100 rounded-lg">
                      <div className="text-xs text-amber-700 mb-1 font-bold">مكتب Yalidine المحدد:</div>
                      <div className="text-amber-900 font-bold">{order.yalidine_desk}</div>
                    </div>
                  )}
                </div>
              </div>
          ...
          <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-6">
            <h2 className="text-lg font-bold text-gray-900 mb-6 flex items-center gap-2">
              🛒 المنتجات المطلوبة
              <span className="text-sm font-normal text-gray-400">({order.order_items.length} منتج)</span>
            </h2>
            ...
            <div className="space-y-4">
              {order.order_items.map((item: any) => {
                ...
                const profit     = buyPrice > 0 ? (sellPrice - buyPrice) * qty : null;
                  ...
                  <div key={item.id} className="rounded-xl border border-gray-100 overflow-hidden">
                    ...
                    <div className="flex gap-4 items-center p-4 bg-gray-50">
                      <div className="w-16 h-16 rounded-lg overflow-hidden bg-white border border-gray-200 flex-shrink-0">
                        <img
                          src={item.products?.images?.[0] || 'https://picsum.photos/seed/sahla/80/80'}
                          alt=""
                          className="w-full h-full object-cover"
                        />
                      </div>
                      <div className="flex-1 min-w-0">
                        <h4 className="font-bold text-gray-900 text-base">{item.products?.name_ar}</h4>
                        {item.variant && (
                      ...
                      <div className="text-left shrink-0">
                        ...
                        <div className="text-lg font-black text-blue-600">{formatDZD(subtotal)}</div>
                      </div>
                    </div>
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 divide-x divide-x-reverse divide-gray-100 border-t border-gray-100">
                      <div className="p-3 text-center">
                        <div className="text-xs text-gray-400 mb-1">سعر البيع / وحدة</div>
                        <div className="font-bold text-gray-900">{formatDZD(sellPrice)}</div>
                      </div>
                      {buyPrice > 0 && (
                        <div className="p-3 text-center">
                          <div className="text-xs text-gray-400 mb-1">سعر الشراء / وحدة</div>
                          <div className="font-bold text-orange-600">{formatDZD(buyPrice)}</div>
                        </div>
                      )}
                      <div className="p-3 text-center">
                      ...
                      {profit !== null && (
                        <div className="p-3 text-center">
                          <div className="text-xs text-gray-400 mb-1">الربح المتوقع</div>
              ...
              <div className="flex justify-between items-center text-sm">
                <span className="text-gray-500 flex items-center gap-2">
                  ...
                  <span className="text-xs px-2 py-0.5 bg-gray-100 rounded-full">
                    {order.shipping_method === 'home' ? 'توصيل منزلي' : 'استلام من المكتب'}
                  </span>
                </span>
                <span className="font-bold text-gray-900">{formatDZD(order.shipping_fee || 0)}</span>
              </div>
              <div className="flex justify-between items-center pt-3 border-t border-gray-200">
                <span className="text-lg font-bold text-gray-900">
                  الإجمالي {order.payment_method === 'cod' ? '(عند الاستلام)' : '(مدفوع)'}
                </span>
            ...
            <div className="space-y-6">
              ...
              <div className="space-y-2">
                ...
                {trackingNumber ? (
                  ...
                  <p className="text-xs text-gray-400">أضف رقم التتبع المقدم من ياليدين ليتمكن الزبون من التتبع.</p>
                )}
              </div>
